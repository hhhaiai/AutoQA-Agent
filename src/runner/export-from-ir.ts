...
/**
 * Wrap navigate code with waitForLoadState for better stability.
 */
...
function generateAssertTextCode(
  ...
  if (typeof visibleNth === 'number' && Number.isInteger(visibleNth) && visibleNth >= 0) {
    const locatorVar = `locator${record.stepIndex ?? 0}`
    return {
      code: `  const ${locatorVar} = page.getByText('${escapeString(text)}');\n  await expect(${locatorVar}.nth(${visibleNth})).toBeVisible();`,
    }
  }
...
function generateAssertElementCode(record: ActionRecord): CodeResult {
  ...
  const locatorCode = record.element!.chosenLocator!.code
  const stepNum = record.stepIndex ?? 0
  const locatorVar = `locator${stepNum}`

  return {
    code: `  const ${locatorVar} = ${locatorCode};\n  await expect(${locatorVar}).toHaveCount(1);\n  await expect(${locatorVar}).toBeVisible();`,
  }
}
...
function generateStepCode(
  ...
  switch (record.toolName) {
    ...
    case 'scroll':
    case 'wait':
      // These don't generate test code (they're for runtime only)
      return { code: '' }

    default:
      return {
        code: `  // TODO: Step ${step.index} - Unsupported tool: ${String(record.toolName)}`,
      }
  }
}
